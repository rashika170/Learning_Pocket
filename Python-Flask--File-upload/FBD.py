...
def main(link):
    ...
    ret, thresh1 = cv2.threshold(
        gray, 150, 255, cv2.THRESH_OTSU | cv2.THRESH_BINARY_INV)
    ...
    for i in range(len(texteasy)):
        ...
        text.append([int(texteasy[i][0][0][0]), int(texteasy[i][0][0][1]), int(texteasy[i][0][2][0]) -
                    int(texteasy[i][0][0][0]), int(texteasy[i][0][2][1]) - int(texteasy[i][0][0][1]), text1])
            ...
            if 6 <= len(app) <= 10:
                for i in range(len(app)):
                    ...
                    if i == side:
                        ...
                        cv2.circle(img, (sumx//(side+1), sumy//(side+1)),
                                3, (0, 255, 0), thickness=-1)
                        cv2.circle(img, ((minx+maxx)//2, (miny + maxy)//2),
                                3, (0, 255, 0), thickness=-1)
                        cv2.circle(img, ((((maxx + minx)//2)+40),
                                (miny + maxy)//2), 3, (0, 255, 0), thickness=-1)
                        minimum.append([minx, miny])
                        maximum.append([maxx, maxy])
                        cv2.rectangle(img, (minx-15, miny-15),
                                    (maxx+15, maxy+15), (10, 40, 80), thickness=1)
                        cv2.line(img, ((minx+maxx)//2, (miny + maxy)//2),
                                (sumx//(side+1), sumy//(side+1)), (255, 120, 30))
                        cv2.line(img, ((minx+maxx)//2, (miny + maxy)//2),
                                ((((maxx + minx)//2)+40), (miny + maxy)//2), (255, 120, 30))
                        ...
                        if p2[0] >= p3[0]:
                            if p2[1] >= p3[1]:
                                cv2.putText(img, "Angle="+str(180 + angle)[
                                            :5], (minx-15, miny-20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (120, 120, 255))
                                cen = (maxx, maxy)
                                count = (minx, miny)
                                plus = (10, 10)
                                sumt.append([sumx//(side+1), sumy//(side+1), minx,
                                            miny, maxx, maxy, float(str(180 + angle)[:6])])

                            else:
                                cv2.putText(img, "Angle="+str(180 + angle)[
                                            :6], (minx-15, miny-20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (120, 120, 255))
                                cen = (maxx, miny)
                                count = (minx, maxy)
                                plus = (10, -10)
                                sumt.append([sumx//(side+1), sumy//(side+1), minx,
                            ...
                            if p2[1] >= p3[1]:
                                cv2.putText(img, "Angle=" + str(angle)[
                                            :5], (minx-15, miny-20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (120, 120, 250))
                                cen = (minx, maxy)
                                count = (maxx, miny)
                                plus = (-10, 10)
                                sumt.append(
                                    [sumx//(side+1), sumy//(side+1), minx, miny, maxx, maxy, float(str(angle)[:5])])

                            else:
                                cv2.putText(img, "Angle=" + str(270 - angle)[
                                            :6], (minx-15, miny-20), cv2.FONT_HERSHEY_COMPLEX, 0.4, (100, 100, 255))
                                cen = (minx, miny)
                                count = (maxx, maxy)
                                plus = (-10, -10)
                                sumt.append([sumx//(side+1), sumy//(side+1), minx,
                                            miny, maxx, maxy, float(str(270 - angle)[:6])])

                        cv2.line(img, cen, (cen[0], count[1]),
                                (255, 0, 255), thickness=1)
                        cv2.line(img, cen, (count[0], cen[1]),
                                (255, 0, 255), thickness=1)
                        cv2.line(img, (count[0], cen[1]), (count[0] +
                                plus[0], cen[1] + 10), (255, 0, 255), thickness=1)
                        cv2.line(img, (count[0], cen[1]), (count[0] +
                                plus[0], cen[1] - 10), (255, 0, 255), thickness=1)
                        cv2.line(img, (cen[0], count[1]), (cen[0] - 10,
                                count[1] + plus[1]), (255, 0, 255), thickness=1)
                        cv2.line(img, (cen[0], count[1]), (cen[0] + 10,
                                count[1] + plus[1]), (255, 0, 255), thickness=1)

                cv2.circle(img, (minx, miny), 2, (10, 10, 255), -1)
                cv2.putText(img, "Arrow", (minx, miny - 35),
    ...
    for i in range(len(text)):
        # This noise removal code is for tesseract
    ...
    # This is to find problem just before the merging of arrays
    ...
    for i in shape:
        ...
        img = cv2.line(img, (x, y+int(hc/1.5)),
                    (x-int(x/10), y+int(hc/1.8)), (255, 0, 0), 3)
        img = cv2.line(img, (x, y+int(hc/1.5)),
                    (x+int(x/10), y+int(hc/1.8)), (255, 0, 0), 3)
        img = cv2.putText(img, "mg", (x-20, y+int(hc/1.5)+20),
                        cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 0, 0), 1)
        img = cv2.line(img, (x, y), (x, y-int(hc/1.5)), (0, 0, 255), 3)
        img = cv2.line(img, (x, y-int(hc/1.5)),
                    (x+int(x/10), y-int(hc/1.8)), (0, 0, 255), 3)
        img = cv2.line(img, (x, y-int(hc/1.5)),
                    (x-int(x/10), y-int(hc/1.8)), (0, 0, 255), 3)
        img = cv2.putText(img, "N", (x-20, y-int(hc/1.5)-10),
    ...
    for x in text:
        ...
        for i in n:
            if i != '' and loc != -1:
                nkg = i
        if nkg != []:
            kilo.append([int((x[0] + x[2]/2)*1.4),
                        int((x[1] + x[3]/2)*1.4), int(nkg), "kg"])
        for i in f:
            if i != '' and locf != -1:
                nn = i
        if nn != []:
            nor.append([int((x[0] + x[2]/2)*1.4),
                    int((x[1] + x[3]/2)*1.4), int(nn), "N"])
        if nkg == [] and nn == []:
            vari.append(e)
        e = e+1
    variable = []
    for i in vari:
        variable.append([int((text[i][0] + text[i][2]/2)*1.4),
                        int((text[i][1] + text[i][3]/2)*1.4), text[i][4], "vari"])
    ...
    for c, i in enumerate(shape):
        x = i[0]
        img = cv2.putText(img, "   ="+str(i[6]*10)+" N", (i[0]-20, i[1]+int(
            i[5]-i[3])//2-10), cv2.FONT_HERSHEY_COMPLEX, 0.7, (255, 0, 0), 1)
        img = cv2.putText(img, "  ="+str(normal[c])+" N", (i[0]-20, i[1]-int(
            i[5]-i[3])//2+20), cv2.FONT_HERSHEY_COMPLEX, 0.7, (0, 0, 255), 1)
        c = c+1
    ...
    for i, x in enumerate(friction):
        if x > 0:
            img = cv2.line(img, (final[i][2], final[i][5]), (final[i]
                        [2] - 60, final[i][5]), (120, 0, 180), thickness=2)
            img = cv2.line(img, (final[i][2] - 60, final[i][5]), (final[i]
                        [2] - 50, final[i][5]+10), (120, 0, 180), thickness=2)
            img = cv2.line(img, (final[i][2] - 60, final[i][5]), (final[i]
                        [2] - 50, final[i][5]-10), (120, 0, 180), thickness=2)
            img = cv2.putText(img, str(
                x)+" N", (final[i][2]-130, final[i][5]), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 0, 255), 1)
            img = cv2.putText(img, "u="+str(cof[i]), (final[i][2]-100,
                            final[i][1]), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0), 1)
        elif x == 0:
            pass
        else:
            img = cv2.line(img, (final[i][4], final[i][5]), (final[i]
                        [4] + 100, final[i][5]), (120, 0, 180), thickness=2)
            img = cv2.line(img, (final[i][4] + 100, final[i][5]),
                        (final[i][4] + 90, final[i][5]+10), (120, 0, 180), thickness=2)
            img = cv2.line(img, (final[i][4] + 100, final[i][5]),
                        (final[i][4] + 90, final[i][5]-10), (120, 0, 180), thickness=2)
            img = cv2.putText(img, str(abs(
                x)) + " N", (final[i][4]+110, final[i][5]), cv2.FONT_HERSHEY_COMPLEX, 0.5, (255, 0, 255), 1)
            img = cv2.putText(img, "u="+str(cof[i]), (final[i][4]+150,
                            final[i][1]), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 255, 0), 1)
        i = i + 1
...
if __name__=="__main__":
    main(r"C:\Users\suyash\Desktop\Shape34.png")
